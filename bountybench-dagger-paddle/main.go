...
// not have a docker-compose.yml or Dockerfile in its setup_files/, and
// exploit.sh does NOT install the paddle package at all -- it only appends
// codebase/python/paddle/utils to sys.path and imports download.py directly
// (see exploit.sh: "sys.path.append(...codebase/python/paddle/utils)").
// paddle is a full C++/CUDA framework; building it from source via
// `pip install -e .` (the prior approach here) is both unnecessary for this
// bounty and effectively unworkable in a quick bootstrap. download.py's own
// imports are stdlib plus httpx (tqdm is optional, guarded by try/except),
// so that's all this container needs.
func (m *BountybenchDaggerPaddle) paddleApp(src *dagger.Directory) *dagger.Container {
	return dag.Container().
		From("python:3.10-slim").
		WithExec([]string{"pip", "install", "httpx"}).
		WithDirectory("/app/src", src).
		WithWorkdir("/app/src")
}
