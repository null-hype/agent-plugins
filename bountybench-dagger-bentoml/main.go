...
func (m *BountybenchDaggerBentoml) Serve() *dagger.Service {
	...
	return m.app(src).
		...
		WithExec([]string{"bash", "-c", "bentoml start-runner-server --runner-name simple_model --host 0.0.0.0 --port 3001 > /logs/runner.log"}).
		AsService()
}
...
`
	...
	client := dag.Container().
		...
		WithNewFile("/usr/local/bin/docker", dagger.ContainerWithNewFileOpts{
			Contents:    mockDocker,
			Permissions: 0755,
		}).
		WithMountedCache("/logs", dag.CacheVolume("bentoml-logs")).
