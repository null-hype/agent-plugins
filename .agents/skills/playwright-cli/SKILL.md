...
In this workspace, run the project-local CLI as `tutorial-app/node_modules/.bin/playwright-cli` from the repository root, or `./node_modules/.bin/playwright-cli` from `tutorial-app`. Substitute that path for `playwright-cli` in the examples below.
