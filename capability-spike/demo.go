...
func runDemo(ctx context.Context) (transcript string, alignedFlags, misalignedFlags []reconcile.Flag, err error) {
	...
	if red.Passed {
		return "", nil, nil, fmt.Errorf("expected worker fact to start red, but it passed")
	}
	diag, ok := diagnostic.Parse(red.Stderr)
	if !ok {
		return "", nil, nil, fmt.Errorf("expected a parseable diagnostic, got:\n%s", red.Stderr)
	}
	...
	if err = supervisor.Decide(ctx, statePath,
		supervisor.Grant{FactID: mainFactID, Vault: mainVault, Approved: true},
		supervisor.Grant{FactID: rejectedFactID, Vault: rejectedVault, Approved: false},
	); err != nil {
		return "", nil, nil, err
	}
	...
	if !green.Passed {
		return "", nil, nil, fmt.Errorf("expected worker fact to pass after approval, got:\n%s", green.Stderr)
	}
	...
	if beforeHash != afterHash {
		return "", nil, nil, fmt.Errorf("worker fact file changed between red and green (%s -> %s); approval must not require editing the fact", beforeHash, afterHash)
	}
	...
	if len(preMaterialization) != 1 || preMaterialization[0].Kind != reconcile.FlagMissingMaterialization || preMaterialization[0].FactID != mainFactID {
		return "", nil, nil, fmt.Errorf("expected exactly one missing-materialization flag for %s before recording the Proton observation, got: %+v", mainFactID, preMaterialization)
	}
	...
	if stillRed.Passed {
		return "", nil, nil, fmt.Errorf("expected explicitly-rejected fact to stay red")
	}
	...
	return t.String(), alignedFlags, misalignedFlags, nil
}
