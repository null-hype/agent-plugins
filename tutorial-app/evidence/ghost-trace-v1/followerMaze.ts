/* CIT-247: vendored (manual copy, no shared module boundary -- this
 * directory's scripts run standalone under plain Node, outside the Vite/
 * Astro build) from ../../src/lesson-farms/follower-maze/followerMaze.ts.
 * Copied verbatim because it is genuinely dependency-free (no
 * `import.meta.glob`, unlike its sibling followerMazeLog.ts), so it runs
 * as-is under `node --experimental-strip-types`. If that source file
 * changes, re-copy it here and re-run capture-ghost-trace-v1.mjs. */
